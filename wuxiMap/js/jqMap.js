...
function initMap() {
	...
	map.addEventListener('touchend', function(e) {
		...
		var c = b;
		if(isContains(b.outerHTML, "juhe_jq.png") || isContains(b.outerHTML, "juhe_kfq.png")) {
			map.zoomIn();
		}
		...
		for(var i = 0; i < data.wuxi.length; i++) {
			...
			marker.addEventListener("click", function(e) {
				...
				var content =
					...
					'<div><input type="button" value="到这里去" style="width:115px;height:30px;" onclick="toPath(' + p.getPosition().lng + ',' + p.getPosition().lat + ',' + '\'' + p.name + '\'' + ')">' +
					'<input type="button" value="在附近找" style="width:115px;height:30px;margin-left:5px;" onclick="toService(' + p.getPosition().lng + ',' + p.getPosition().lat + ')"></div>' +
					'</div></div></div>';
	...
	$.getJSON('wuxi.json', function(data) {
		for(var i = 0; i < data.kaifaqu.length; i++) {
			...
			marker.addEventListener("click", function(e) {
				...
				var content =
					'<div style="position: relative; width: 240px; height: 100px;}">' +
					'<div onclick="toView1(' + p.url + ',' + '\'' + p.name + '\'' + ')">' +
					'<img style="width:240px;height:100px;" src="' + p.image + '"></img>' +
					'<span style="position:absolute;bottom:0;left:0;width:240px;height:18px;background: #000;opacity: 0.5;color:white;">查看全景--></span>' +
					'<div>' +
					'</div>' +
